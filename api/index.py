...
# Required for Vercel serverless Python runtime
handler = app
...
def google_trends(query: str = Query(...), api_key: str = Depends(verify_api_key)):
    try:
        ...
        analysis = {
            "interest": interest,
            "related": related,
            "insights": "Google trends fetched"
        }
        supabase.table("trends").insert({"platform": "google", "query": query, "raw": interest, "analysis": analysis}).execute()
...
def reddit_trends(query: str = Query(...), api_key: str = Depends(verify_api_key)):
    try:
        ...
        url = f"https://www.reddit.com/search.json?q={query}&sort=hot&limit=10"
        r = requests.get(url, headers=headers, timeout=10)
        r.raise_for_status()
        data = r.json()
        posts = data.get('data', {}).get('children', [])
        raw = [p['data'] for p in posts]
        analysis = {"insights": "Reddit hot posts fetched"}
        supabase.table("trends").insert({"platform": "reddit", "query": query, "raw": raw, "analysis": analysis}).execute()
